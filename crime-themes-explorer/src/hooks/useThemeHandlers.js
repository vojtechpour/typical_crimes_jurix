...
export const useThemeHandlers = ({
  ...
  const revertThemeMove = useCallback(
    async ({ candidateTheme, fromTheme, toTheme }) => {
  ...
  const revertCandidateThemeChange = useCallback(
    async ({ action, candidateTheme, oldName, newName, theme }) => {
      switch (action) {
        case "rename":
  ...
  const handleSuggestionMoveMultiple = useCallback(
    async ({ candidates, fromTheme, toTheme }) => {
  ...
  const handleSuggestionRename = useCallback(
    async ({ currentName, suggestedName, theme }) => {
  ...
  const handleSuggestionThemeRename = useCallback(
    async ({ currentName, suggestedName }) => {
  ...
  const handleSuggestionCreateTheme = useCallback(
    async ({ newThemeName, candidatesToMove, fromTheme, fromThemes }) => {
      try {
  ...
  const handleSuggestionMergeThemes = useCallback(
    async ({ theme1, theme2, newThemeName }) => {
      ...
      // Update local state
      setAllThemes((prev) => {
        const theme1Obj = prev.find((t) => t.name === theme1);
        const theme2Obj = prev.find((t) => t.name === theme2);
        ...
        return [
          ...prev.filter((t) => t.name !== theme1 && t.name !== theme2),
      ...
      if (selectedLeftTheme === theme1 || selectedLeftTheme === theme2) {
        ...
        const theme1Obj = setAllThemes.find?.((t) => t.name === theme1);
        const theme2Obj = setAllThemes.find?.((t) => t.name === theme2);
        if (theme1Obj && theme2Obj) {
          setLeftCandidateThemes([
            ...theme1Obj.candidateThemes,
            ...theme2Obj.candidateThemes,
          ]);
  ...
  const handleSuggestionMergeCandidates = useCallback(
    async ({ candidate1, candidate2, newName, theme }) => {
